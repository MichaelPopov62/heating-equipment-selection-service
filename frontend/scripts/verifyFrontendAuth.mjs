...
const clerkLazyRoot = readSrc('auth/ClerkLazyRoot.tsx');
const shouldLoadClerkForPath = readSrc('auth/shouldLoadClerkForPath.ts');
...
assert.match(appTsx, /ClerkLazyRoot/);
assert.doesNotMatch(appTsx, /ClerkProviderWithRouter/);
assert.match(clerkLazyRoot, /shouldLoadClerkForPath/);
assert.match(clerkLazyRoot, /lazy\(\(\) =>/);
assert.match(shouldLoadClerkForPath, /paths\.login/);
assert.match(shouldLoadClerkForPath, /paths\.signUp/);
assert.match(shouldLoadClerkForPath, /paths\.projects/);
assert.match(shouldLoadClerkForPath, /paths\.adminFeedback/);
...
assert.match(clerkAuthProviderInner, /useClerkAuth/);
assert.match(clerkAuthProviderInner, /resolveClerkJwtTemplateForApi/);
assert.match(clerkAuthProviderInner, /getToken\(\{ template: jwtTemplate \}\)/);
assert.doesNotMatch(clerkAuthProviderInner, /return await getToken\(\);/);
assert.match(clerkAuthProviderInner, /setProjectsAuthTokenGetter/);
assert.match(authProvider, /PublicAuthProviderInner/);
assert.match(authProvider, /ClerkLoadContext/);
assert.doesNotMatch(authProvider, /@clerk\/clerk-react/);
