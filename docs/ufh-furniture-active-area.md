...
L_петли = S_акт / шаг_мм × 1000      (на одну петлю после деления по maxUfhLoopLengthM)
```
...
|---------|----------------|-------------------|
| Петля ТП в стяжке | `loopLengthM` = `heatedAreaM2 / шаг / loopsCount` | **да** |
...
Форма укладки в полу (меандр, улитка) в backend **не задаётся** — учитывается только длина петли.
