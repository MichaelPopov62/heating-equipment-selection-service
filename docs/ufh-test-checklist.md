# Чеклист тестирования: контур ТП

Ручная проверка режимов ТП, пресетов Mongo (`underfloor_heating_presets`), `ufh_only` и гидравлики контура пола.
...
- [ ] `GET /api/v1/presets/underfloor-heating/modes` — 2 режима
...
- [ ] Радиаторы не подбираются (`skippedReason: ufh_only`)
- [ ] `matching.boiler.requiredKw` от `totalHeatFluxUpWatts` (с запасом), не от envelope `heatLoss`
- [ ] При дефиците покрытия — `WARN_UFH_COVERAGE_LOW_UFH_ONLY`

---

## Сценарий 5 — Схема распределения
...
## Сценарий 6 — Мебель на полу ТП (S_meb)
...
- [ ] `WARN_UFH_COVERAGE_LOW` при `heatFluxCoverageRatio` &lt; 0.95
- [ ] `ufhTerminalControl=unibox` при area≤20 → SKU в matching.uniboxes
...
- [ ] `lineEconomy` 75/65, `lineEfficient` 55/45 (кроме `ufh_only`)
...
- Рабочий график 95/85 (только legacy CalcInput API)
