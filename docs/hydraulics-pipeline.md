...
4. **Порядок в buildReport** — `matchEquipment` → резолв `distributionPreset` ТП → **`pickManifolds`** (`matching.manifolds`, soft-fail `ok`, см. [`manifold-matching.md`](manifold-matching.md)) → **`pickUniboxes`** (`matching.uniboxes`, см. [`unibox-matching.md`](unibox-matching.md)) → `buildHydraulicsSnapshots` → `validateHydraulicsPipelineInput` → `runHydraulicsPipeline`. Сбой коллекторов (`ok: false`) не прерывает отчёт: унибоксы и гидравлика продолжаются.
