...
| Файл | Содержание |
|------|------------|
| `surveySteps.ts` | `SURVEY_STEPS` (`object` → `warmFloor` → `rooms` → … → `hydraulics` → `technicalResult` → `dataReference`), `SURVEY_STEP_NAV_ITEMS`, `isSurveyStep`, `isCalcApiBarStep`, `surveyStepGlobalMetaTitle` |
