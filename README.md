...
Сервис закрывает задачу подбора отопления для частного дома или квартиры без ручного сведения теплопотерь, ГВС и каталога оборудования. Пользователь заполняет анкету (помещения, ограждения, вентиляция, ГВС, гидравлика); по геолокации подтягивается климат, считаются нагрузки и формируется JSON-отчёт с подбором котла, радиаторов, ТП, коллекторов и сметы. Есть сохранение проектов, публичная share-ссылка и PDF для клиента.
