...
function baseDto(overrides = {}) {
  ...
  const dto = {
    ...
    circuits: {
      ...
      underfloor: {
        deltaTK: 10,
        aggregate: { heatLoadWatts: 8000, flowRateM3PerHour: 1.2 },
...
// Сценарий 1: прямое подключение — sum, не max
{
  ...
  assert.equal(r.boilerPumpDesignFlowM3PerHour, 2.2);
  assert.equal(r.zones.find((z) => z.zoneId === 'boiler_primary')?.designFlowM3PerHour, 2.2);
  console.log('OK scenario 1 direct mixed: Q=2.2 m³/h');
}
...
// Сценарий 2: смесительный узел — два насоса
{
  ...
  assert.ok(main && ufh);
  assert.equal(ufh.designFlowM3PerHour, 1.2);
  assert.ok(main.designFlowM3PerHour > 0 && main.designFlowM3PerHour < 2.2);
  assert.notEqual(main.designFlowM3PerHour, 1.2);
...
// Сценарий 3: гидрострелка — primary + secondary zones
{
  ...
  assert.ok(r.primaryMainLineFlowM3PerHour >= 2.2 * 1.12 - 0.01);
...
// Сценарий 4: БКН — max(Q_heating, Q_dhw), не sum
{
  ...
  const main = r.zones.find((z) => z.zoneId === 'boiler_primary');
  assert.ok(main);
  assert.ok(main.designFlowM3PerHour > 2.2);
